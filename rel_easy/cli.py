...
from rel_easy.util import (
    find_package_paths,
    path_to_package_candidate,
    create_setup_py,
    get_or_prompt,
    separate_name_and_email,
    build_and_clean,
    pypi_upload,
    create_version_file,
    temp_cwd,
    pypirc_parse_config,
    pip_add_extra_index_url_to_conf,
    pip_get_conf_servers,
    pip_get_conf_path,
...
def click_promptChoice(*choices, **kwds):
    ...
    # print("D:",default)
    result = click.prompt(
        prompt, type=int, default=default, show_default=default is not None
    )
...
@click.option(
    "-p", "--package_dir", type=click_package, default=list(find_package_paths("."))
)
@click.option("-g", "--git-hash", is_flag=True, default=False)
@require_init
def rev(major, minor, build, extra, **kwds):
    package, package_dir = (
        kwds["package_dir"]["package"],
        kwds["package_dir"]["package_dir"],
    )
    old_dir = os.getcwd()
    ...
    other_ver.version_tuple = tuple(
        a + b for a, b in zip([major, minor, build], other_ver.version_tuple)
    )
    other_ver.version = ".".join(map(str, other_ver.version_tuple))
    if kwds.get("git_hash"):
        sha_hash = os.popen("git log --pretty=%H -1").read()
    create_version_file(
        package["package"], package["package_dir"], str(other_ver), sha_hash=sha_hash
    )
...
@click.argument(
    "which", type=click.Choice(VER_NAMES, case_sensitive=False), default="build"
)
@click.option(
    "-p", "--package_dir", type=click_package, default=list(find_package_paths("."))
)
...
def set_ver(major, minor, build, extra, sha1=False, **kwds):
    ...
    if not os.path.exists(
        os.path.join(package["package_dir"], package["package"], "version.py")
    ):
        executable = click.get_current_context().command_path.split(" ", 1)[0]
        raise RuntimeError("You MUST run `%s init` first" % executable)
    try:
        ...
        major, minor, build, extra = _get_version_overrides(
            major, minor, build, extra, **kwds
        )
    ...
    if sha1:
        ...
        git_hash = ""
    create_version_file(
        package["package"], package["package_dir"], str(ver), sha_hash=git_hash
    )
...
@click.option(
    "-d", "--description", prompt=True, default="a short description", type=str
)
...
@click.option(
    "-r", "--add-github-release-action", is_flag=True, prompt=True, default=True
)  # type=click.Choice("Yn",case_sensitive=False), default="y", prompt=True)
@click.option(
    "-l", "--add-github-lint-action", is_flag=True, prompt=True, default=True
)  # type=click.Choice("Yn",case_sensitive=False), default="y", prompt=True)
...
def init(major, minor, build, extra, **kwds):
    ...
    if not os.path.exists(setup_path):
        ...
        if import_stmt not in setup_txt:
            r = click.prompt(
                "It looks like we should update setup.py to use the version?",
                default="y",
                type=click.Choice("Yn", case_sensitive=False),
            )
            if r.lower() == "y":
...
def install_github_release_action(pkgDir, pkgName):
    ...
    templ_path = os.path.join(
        os.path.dirname(__file__), "DATA", "github-deploy-action-yml.tmpl"
    )
    ...
    os.system(
        "git add {package_dir}/.github/workflows/deploy.yml".format(package_dir=pkgDir)
    )
...
def _initialize_setup_py_kwds(kwds):
    data = dict(
        pkg_name=get_or_prompt(
            kwds,
            "--package-name",
            "yes",
            kwds["package"].replace("_", "-"),
            click.prompt,
        ),
        pkg_desc=get_or_prompt(
            kwds, "--description", "yes", "This is just some project", click.prompt
        ),
        pkg_author=get_or_prompt(
            kwds, "--author", "yes", "anony mouse <anyone@email.com>", click.prompt
        ),
    )
...
def del_install_server(url=None, yes=False):
    ...
    if not url:
        ...
        urls = list(
            filter(None, urls.get("extra-index-url", []) + urls.get("index-url", []))
        )
        if not urls:
...
def pypy_get_alias_choice_prompt(prompt1, prompt2):
    ...
    result = click.prompt(
        prompt2, show_choices=False, type=click.Choice(choices, case_sensitive=False)
    )
...
def pypi_wizard_root_prompt():
    ...
    result = click.prompt(
        "Choose an item to Edit or N,D,X",
        show_choices=False,
    ...
    if result.isdigit():
        ...
        result = pypy_get_alias_choice_prompt(
            "SELECT ITEM TO DELETE (or 'x' to exit)", "Choose an item to DELETE"
        )
        action = {"action": "delete", "target": result}
    return action


def pypi_edit_or_create_prompt(
    alias=None, repository=None, username=None, password=None
):
...
@cli3.command(
    "del-server",
    help="setup and configure your deploy credentials and servers for pypi",
)
...
def print_server_line(server, data, i):
    ...
    click.echo(
        "  {ct_fmt}{0:<{1}s}| {2}".format(server, sz, url, ct_fmt=ct_fmt.format(i))
    )
